...
class GammaSaleAdapter(BaseSaleAdapter):
    ...
    def cache_data(self):
        ...
        self.df.loc[:, f"{self.dt_col}_str"] = self.df[self.dt_col].map(lambda x: x.strftime(self.dt_format))
    ...
    def _read_df(self):
        ...
        sku_info_list = self.config['supply_network_config'].get_sku_of_store(self.store_idx)
        data_list = []
...
class NoisyGammaSaleAdapter(GammaSaleAdapter):

    def __init__(self, config, noisy_level=1.0):
        self.noisy_level = noisy_level
        super(NoisyGammaSaleAdapter, self).__init__(config)

    def _read_df(self):
        os.makedirs('data/NoisyGammaRetail/', exist_ok=True)
        file_name = f"data/NoisyGammaRetail/store{self.store_idx+1}.csv"
        if os.path.exists(file_name):
            return pd.read_csv(file_name, parse_dates=[self.dt_col])
        sku_info_list = self.config['supply_network_config'].get_sku_of_store(self.store_idx)
        data_list = []
        for sku_info in sku_info_list:
            ...
            sku_price = sku_info['price']
            for i in range(self.total_span):
                weekday = (self.start_dt+timedelta(i)).weekday()
                noisy_level = np.random.randint(6, 14) / 10.0 + (0 if weekday < 4 else (np.random.random()+self.noisy_level))
                demand = int(np.random.gamma(sale_gamma*noisy_level))
                data_list.append([sku_name, self.start_dt+timedelta(i), demand, sku_price])
        df = pd.DataFrame(data_list, columns=[self.id_col, self.dt_col, self.sale_col, self.sale_price_col])
        df.to_csv(file_name)
        return df
