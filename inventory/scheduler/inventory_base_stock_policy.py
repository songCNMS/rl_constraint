...
class ConsumerBaseStockPolicy(ConsumerBaselinePolicy):
    ...
    def _find_source(self, f_state_info):
        ...
        base = self.base_stock[len(self.base_stock) // 2]
        # whether replenishment point is reached
        if booked_inventory[most_needed_product_id] >= base:
            return 0
        
        sale_gamma = f_state_info['sale_mean']
        consumer_quantity = self.config['supply_network_config'].get_consumer_quantity_action((base - booked_inventory[most_needed_product_id]) / sale_gamma)
