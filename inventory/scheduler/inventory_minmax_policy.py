...
class ConsumerMinMaxPolicy(ConsumerBaselinePolicy):
    ...
    def _get_consumer_quantity(self, f_state_info):
        ...
        return self.config['supply_network_config'].get_consumer_quantity_action(consumer_quantity)
    ...
    def _find_source(self, f_state_info):
        ...
        vlt_buffer_days = 10 + 7 * ( self.config['supply_network_config'].env_config['total_echelons'] - f_state_info['echelon_level'] )
        ...
        R = 3*r
        consumer_quantity = self.config['supply_network_config'].get_consumer_quantity_action(int((R - r) / sale_mean))
