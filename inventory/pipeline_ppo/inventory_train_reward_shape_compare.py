...
def create_ppo_trainer(env, config, echelon, n_iterations, args):
    ...
    ext_conf.update({
            ...
            "exploration_config": {
                ...
                "random_timesteps": args.rollout_fragment_length*args.batch_size*5,
            },
            "multiagent": {
...
def record_eval(reward_shape, eval_mean_reward, eval_mean_reward_list, i):
    if reward_shape:
        writer.add_scalar('eval/eval_balance_stat/with_reward_shape', eval_mean_reward, i)
        for r in eval_mean_reward_list:
            writer.add_scalar('eval/eval_balance_hist/with_reward_shape', r, i)
    else:
        writer.add_scalar('eval/eval_balance_stat/without_reward_shape', eval_mean_reward, i)
        for r in eval_mean_reward_list:
            writer.add_scalar('eval/eval_balance_hist/without_reward_shape', r, i)

def train_ppo(_env, n_iterations, step_iterations, ppo_trainer, workdir, is_constraint_imposed, reward_shape):
    mean_reward_hist = []
    ...
    for i in range(n_iterations):
        ...
        for j in range(step_iterations):
            ...
            record_eval(reward_shape, policy_reward_mean, policy_reward_mean_list, i)
...
def render(config, trainer, args, is_constraint_imposed, workdir, is_render=False):
    ...
    track_metric_list = []
    if is_render:
        tracker_metric, metric_list = tracker.run_and_render(workdir, facility_types=[SKUStoreUnit, SKUWarehouseUnit])
...
if __name__ == "__main__":
    ...
    env_config_for_rendering['reward_shape'] = True
    ...
    if int(stop_iters[0]) > 0:
        ...
        retailer_ppo_trainer_woc = create_ppo_trainer(env, env_config_for_rendering, total_echelon-1, int(stop_iters[0]), args)
        retailer_ppo_trainer_woc, mean_rewards, _ = train_ppo(env, int(stop_iters[0]), args.step_iters, retailer_ppo_trainer_woc, workdir+'/without_reward_shape/', False, reward_shape)
        retailer_ppo_trainer_woc.save(f"{checkpoint_path}/without_reward_shape/")
        train_hist.append(mean_rewards)
        train_hist_labels.append('mean_reward_without_reward_shape')
    ...
    if int(stop_iters[1]) > 0:
        ...
        retailer_ppo_trainer = create_ppo_trainer(env, env_config_for_rendering, total_echelon-1, int(stop_iters[1]), args)
        retailer_ppo_trainer, mean_rewards, _ = train_ppo(env, int(stop_iters[1]), args.step_iters, retailer_ppo_trainer, workdir+'/with_reward_shape/', False, reward_shape)
        retailer_ppo_trainer.save(f"{checkpoint_path}/with_reward_shape/")
        train_hist.append(mean_rewards)
        train_hist_labels.append('mean_reward_with_reward_shape')
